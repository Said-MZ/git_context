...
exports.deriveChangeIntent = deriveChangeIntent;
...
function scoreCluster(tokens, cluster) {
    let score = 0;
...
function deriveChangeIntent(commits) {
    if (commits.length === 0) {
        return {
    ...
    for (const { cluster, score } of clusterScores) {
        ...
        for (const commit of commits) {
            if (assignedCommits.has(commit.sha))
                continue;
    ...
    const relevantKeywords = topCluster.cluster.keywords.filter((kw) => allTokens.some((token) => token.includes(kw) || kw.includes(token)));
...
function extractCommonNouns(text) {
