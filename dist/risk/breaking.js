...
function extractExports(content) {
    ...
    const declarations = content.matchAll(/export\s+(?:const|let|var|function|class|interface|type|enum)\s+(\w+)/g);
...
function detectBreakingChanges(diffHunks) {
    const changes = [];
    const seen = new Set();
    for (const hunk of diffHunks) {
        // Skip non-source files
        if (!/\.(ts|tsx|js|jsx|mjs|cjs)$/.test(hunk.file))
            continue;
