import { FileChange, DiffHunk, DataLayerRisk } from '../types';
...
const SQL_OPERATION_PATTERNS: { pattern: RegExp; operation: string }[] = [
...
const ORM_PATTERNS: { pattern: RegExp; operation: string }[] = [
  // Prisma
...
export function detectDataLayerRisks(
  fileChanges: FileChange[],
  diffHunks: DiffHunk[]
): DataLayerRisk[] {
  const risks: DataLayerRisk[] = [];
  const fileRisksMap = new Map<string, { tables: Set<string>; operations: Set<string> }>();
